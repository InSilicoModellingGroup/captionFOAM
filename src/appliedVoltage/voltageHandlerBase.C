...
defineTypeNameAndDebug(voltageHandlerBase, 0);
defineRunTimeSelectionTable(voltageHandlerBase, voltageWaveform);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //
voltageHandlerBase::voltageHandlerBase
(
    const fvMesh& mesh, 
    const dictionary& dict, 
    volScalarField& voltExt, 
    const volScalarField& voltExtAmp
) 
:
    mesh_(mesh),
    dict_(dict),
...
autoPtr<voltageHandlerBase> voltageHandlerBase::NewVoltageWaveform
(
    const fvMesh& mesh,
    const dictionary& dict,
    volScalarField& voltExt,
...
{
    const word voltageType = dict.get<word>("type");

    auto* ctorPtr = voltageWaveformConstructorTable(voltageType);

    if (!ctorPtr)
    {
        FatalIOErrorInLookup(dict, "voltage waveform", voltageType, *voltageWaveformConstructorTablePtr_)
            << exit(FatalIOError);
    }

    return autoPtr<voltageHandlerBase>(ctorPtr(mesh, dict, voltExt, voltExtAmp));
}

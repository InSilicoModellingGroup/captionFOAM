...
:
    ...
    k_(reactions_.size()),
    reactionProcess_(reactions_.size())
{
    ...
    label j = 0;
    for (const entry& reaction : reactionsSubDict)
    {
        const word type
        (
            reaction.dict().lookup("type")
        );

        const word process
        (
            reaction.dict().lookupOrDefault<word>("process", "undefined")
        );
        reactionProcess_[j] = process;
        ...
        (
            ...
            (
                ...
                mesh_,
                dimensionedScalar("zero", dimensionSet(0, -3*(sumReactants(j)-1), -1, 0, 0, 0, 0), scalar(0))
            )
        );
...
// TODO: see of Ri can be a simple dimensionedScalar instead of a registry object with false
